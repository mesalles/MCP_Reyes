...
async def lifespan(app: FastAPI):
    ...
    # Startup
    logger.info("Starting HTTP MCP Reyes Server...")
    ...
    logger.info("HTTP MCP Reyes Server stopped")
...
async def root():
    ...
    return {
        "name": "MCP Reyes Server",
        "version": "1.0.0",
        "description": "HTTP-only access to Reyes tools via MCP protocol",
        "endpoints": {
...
#                         }
            ...
            return {
                ...
                "result": {
                    "resources": [
                        {
                            "uri": "uji://api/info",
                            "name": "CCN-CERT Reyes tools API",
                            "description": "Information about CCN-CERT Reyes tools API endpoints and usage",
            ...
            if uri == "uji://api/info":
                content = json.dumps({
                    "name": "CCN-CERT Reyes tools API",
                    "description": "Access to CCN-CERT Reyes tools",
                    "version": "1.0.0",
                    "remote_access": True,
                    "endpoints": {
...
def main():
    ...
    parser = argparse.ArgumentParser(description="HTTP MCP Reyes Server")
    ...
    logger.info(f"Starting HTTP MCP Reyes Server on {args.host}:{args.port}")
